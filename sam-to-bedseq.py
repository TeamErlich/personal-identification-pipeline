...
See --bedseq-help for more details about the output format.
    ...
    parser.add_argument('--allow-multiple-exp-starts', action="store_true",
                        help='accept timing file containing multiple experiment' \
                             'start times - the earliest time will be used for' \
                             'all reads')
...
def get_timing_file_earliest_exp_start_time(filename):
    """Scan the timing file, return the earliest experiment-start-time (field 4).
    """
    min_exp_start_time = None
    ...
    try:
        ...
        for linenum,line in enumerate(f):
            ...
            if min_exp_start_time is None:
                min_exp_start_time = experiment_start_time
            if experiment_start_time < min_exp_start_time:
                min_exp_start_time = experiment_start_time
    ...
    if min_exp_start_time is None:
        sys.exit("Input error in timing file (%s): file is empty?"  % (filename))

    return min_exp_start_time



def load_timing_file(filename, allow_multiple_exp_starts=False):
    """
    ...
    if allow_multiple_exp_starts:
        first_experiment_start_time = get_timing_file_earliest_exp_start_time(filename)
    ...
    try:
        ...
        for linenum,line in enumerate(f):
            ...
            if allow_multiple_exp_starts:
                experiment_start_time = first_experiment_start_time
            else:
                ...
                if first_experiment_start_time != experiment_start_time:
                    sys.exit(err + "timing file contains exp-start-time from " \
                             "several different runs. aborting. Use --allow-multiple-exp-starts " \
            ...
            if unix_timestamp_end < experiment_start_time:
                sys.exit(err + "invalid unix timestamps: experiment_start_time (%s) " \
                         "> unix_timestamp_end (%s) [allow-multiple-exp-starts: %s]" \
                         % ( str(experiment_start_time), str(unix_timestamp_end),
                             str(allow_multiple_exp_starts) ))
...
if __name__ == "__main__":
    ...
    if args.times:
        timing = load_timing_file(args.times,
                     allow_multiple_exp_starts=args.allow_multiple_exp_starts)
        if args.debug_times:
            pprint(timing)
            sys.exit()
